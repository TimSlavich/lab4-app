...
export class ServiceComponent {
  ...
  SeriesCalculate(x: string, iterations: string) {
    this.seriesResult = this.SeriesService.getSeries(parseFloat(x), parseFloat(iterations));
    this.visible[1] = true;
  ...
  RecursiveCalculate(x: string, iterations: string) {
    this.recurciveResult = this.RecursiveService.getSeries(parseFloat(x), parseFloat(iterations));
    this.visible[2] = true;
  ...
  AllCalculate() {
    ...
    for (let x = -3 * Math.PI; x <= 3 * Math.PI; x += 1) {
      this.series.set(x.toFixed(2), this.SeriesService.getSeries(x, iterations).toFixed(2));
    }

    this.seriesRec.clear();

    for (let x = -3 * Math.PI; x <= 3 * Math.PI; x += 1) {
      this.seriesRec.set(x.toFixed(2), this.RecursiveService.getSeries(x, iterations).toFixed(2));
    }
